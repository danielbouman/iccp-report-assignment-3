...
The harmonic oscillator potential is defined as $V(r) = \frac{1}{2}m\omega^2r^2$, with $r$ the distance from some point in space. It is known from the classical harmonic oscillator that a particle would oscillate with a certain frequency in space. This behaviour can also be seen in the quantum analog of the harmonic oscillator. A simulation of this potential was performed (time evolution is shown in figure \ref{fig:harmonicpotential}) by given a Gaussian wave packet with an offset a momentum in the positive x-direction in a harmonic potential with $\omega = $\todo[inline]{waarde toevoegen voor omega toevoegn en fatsoenlijk engels maken van deze alinea}.
...
\end{figure} Figure \ref{fig:crosssection} shows a cross-section along the x-axis at $y = 26.5$, $t = 60$ \todo[inline]{Dit zegt niks zonder assen en tijdsindicatie bij de figuren}, $\tau = 0.1$ of the probability distribution is seen in the figure below. The probability distribution is again normalized to find the conditional distribution:
